...
export function ensureDbReady(): Promise<void> {
  if (dbSource !== "pglite") {
    // Still seed admin when Neon is configured (deploy / configured sandbox).
    return import("@/lib/auth/seed-admin.server")
  ...
  return getSql()
    .then(() => import("@/lib/auth/seed-admin.server"))
    .then((m) => m.ensureAdminUser())
    .catch((err) => {
      console.error("[db] bootstrap/seed failed:", err);
    });
}
